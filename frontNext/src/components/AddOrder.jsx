...
export default function AddOrder() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://neopixel3d.ru/api/orders', {
        method: 'POST',
...
<div className='text-white flex mx-auto  p-[10px] justify-center laptop:max-w-7xl laptop:mb-[140px]'>
    <div className='text-lg text-white font-HelveticaNeueCyr font-semibold text-center mb-6 laptop:text-5xl' name="order">ОФОРМИТЬ ПРЕДЗАКАЗ НА ПРИНТЕР HELIODORUS</div>
</div>
<div className='laptop:flex laptop:justify-center'>
    <form className='mx-4' onSubmit={handleSubmit}>
        <div className='grid grid-cols-1 gap-4 font-inter laptop:flex'>
            ...
            <div className="form-group mb-6 flex-auto w-25 pl-[20px]">
              <input type="text" value={printername} className="w-full form-control pl-[20px] laptop:h-[75px] text-xl font-normal text-gray-700 bg-[#1E1F21] bg-clip-padding border border-solid border-gray-300 rounded-full transition ease-in-out m-0 focus:text-gray-700 focus:bg-[#1E1F21] focus:border-blue-600 focus:outline-none" id="exampleInput8" disabled />
            </div>
            <div className="form-group mb-6 flex-auto w-25 pl-[20px]">
                <input name="description" value={description} onChange={handleChange}   className="w-full form-control pl-[20px] laptop:h-[75px] text-xl font-normal text-gray-700 bg-[#1E1F21] bg-clip-padding border border-solid border-gray-300 rounded-full transition ease-in-out m-0 focus:text-gray-700 focus:bg-[#1E1F21] focus:border-blue-600 focus:outline-none" id="exampleInput8" placeholder="Дополнительно" />
            </div>
